...
def read_damage():
    ...
    df = pd.read_csv('damage_variables.csv')
    df_arr = df.to_numpy()
    return df_arr
...
def get_dam_var(dam_var):
    ...
    var1 = f"pos_x = {dam_var[0]}"
    var2 = f"pos_y = {dam_var[1]}"
    var3 = f"theta = {dam_var[2]}"
    var4 = f"c = {dam_var[3]}"
    var5 = f"r = {dam_var[4]}"
    var6 = f"n_dam = {dam_var[5]}"
    struct_var = (var1,var2,var3,var4,var5, var6)
    return struct_var
...
def rename_output(model_num):
    ...
    for i in os.listdir():
        if i != 'APDL_Skin_Damage.txt' and i != 'APDL_Skin_Variables.txt' and not i[0].isnumeric():
            os.rename(i, f'{model_num}_{i}')

def write_var(dam_var, struct_prop, mat_prop):
    '''Writes APDL_Skin_Variables.txt file for APDL input'''

    dam_var = get_dam_var(dam_var)
    struct_prop = get_struct_prop(struct_prop)
    mat_prop = get_mat_prop(mat_prop)

    var_file = open('APDL_Skin_Variables.txt','w')
...
def run_model(dam_var, struct_prop, mat_prop): 
    ...
    os.chdir(os.path.join(cwd,'skin_damage')) 

    delete_lock()
    write_var(dam_var, struct_prop, mat_prop)

    os.system("\"C:\\Program Files\\ANSYS Inc\\ANSYS Student\\v212\\ansys\\bin\\winx64\\MAPDL.exe\" -b -j output -dir C:\\Users\\Ian\\Desktop\\Unifei\\GEMEC\\4_Programas\\clustering\\skin_damage -i C:\\Users\\Ian\\Desktop\\Unifei\\GEMEC\\4_Programas\\clustering\\skin_damage\\APDL_Skin_Damage.txt -o C:\\Users\\Ian\\Desktop\\Unifei\\GEMEC\\4_Programas\\clustering\\skin_damage\\Model_Output.out")
